...
### [Transaction Costs & The Economics of Governance](#4)
### [Asset Specificity](#16)
### [A Game-Theoretic Hold-Up Model](#30)
### [The Double-Marginalization Problem](#38)
...
# Transaction Costs & The Economics of Governance
...
.left-column[
.center[
![:scale 80%](../images/johnrcommons.jpg)
...
- A contract between two parties constitutes a .hi[“fundamental transformation”] from *ex ante* competitive market to an *ex post* bilateral monopoly
  - Two parties depend on one another’s performance to jointly capture the gains from exchange (“quasi-rents” of cooperation)
...
> “Indeed, .hi[in some cases and for some purposes, nearly the whole income of a business may be regarded as a quasi-rent], that is an income determined for the time by the state of the market for its wares, .hi[with but little reference to the cost of preparing for their work] the various things and persons engaged in it. In other words it is a .hi[composite quasi-rent divisible among the different persons in the business by bargaining], supplemented by custom and by notions of fairness...Thus the head clerk in a business has an acquaintance with men and things, the use of which he could in some cases sell at a high price to rival firms. But in other cases .hi[it is of a kind to be of no value save to the business in which he already is; and then his departure would perhaps injure it by several times the value of his salary], while probably he could not get half that salary elsewhere,”  (VI.viii.35).

]
]
.source[Marshall, Alfred, 1870, *Principles of Economics*]
...
.smallest[
> “Coase's fundamental insight [was] that transaction, coordination, and contracting costs must be considered explicitly in explaining the extent of vertical integration...[We] explore one particular cost of using the market system-.hi[the possibility of postcontractual opportunistic behavior],” (p.297)

> “The particular circumstance we emphasize as likely to .hi[produce a serious threat of this type of reneging on contracts is the presence of appropriable specialized quasi rents. After a specific investment is made and such quasi rents are created, the possibility of opportunistic behavior is very real]. Following Coase's framework, .hi[this problem can be solved in two possible ways: vertical integration or contracts],” (p.298)
...
- .hi[“Asset specificity”]: degree to which an asset has alternative valuable uses outside a specific use
...
.smaller[
> “Transaction cost economics maintains that the principal factor that is responsible for transaction cost differences among transactions is variations in asset specificity. Transactions that are supported by non-specific (redeployable) investments are ones for which neoclassical analysis is well-suited to deal. As a condition of asset specificity becomes more important, however, exchange relations take on a progressively stronger bilateral trading character. The reason is that parties to such trades have a stake in preserving the continuity of the relationship,” (p. 367).

]

.source[Riordan, Michael H and Oliver E Williamson, 1985, “Asset Specificity and Economic Organization,” *International Journal of Industrial Organization* 3: 365-378.]
...
.pull-left[
.quitesmall[
- $\color{red}{\Delta G}$: difference in governance cost of organization (firm) vs. market
  - $\Delta G(0)$: advantages of markets $>$ costs of asset specificity
  - declines as assets are more specific (market contracts become disadvantageous beyond $k_1)$
]
]
.pull-right[
```{r}
dg <- function(x){2.5-x^{1.5}}
dc <- function(x){2/x}

total <- function(x){dg(x) + dc(x)}

ggplot(data = tibble(x = c(1,2)))+
  aes(x = x)+
  stat_function(fun = dg, color = "red")+
  geom_label(x = 2.75, y = dg(2.75), color = "red", label = expression(Delta*G))+
  geom_vline(xintercept = 0, size = 0.5)+
  geom_hline(yintercept = 0, size = 0.5)+
  scale_x_continuous(breaks = c(0, 1.85),
                     labels = c(0, expression(k[1])),
                     limits = c(0,3),
                     expand = c(0,0))+
  scale_y_continuous(breaks = c(0, 2.5),
                     labels = c(0, expression(Delta*G[0])),
                     limits = c(-3,3),
                     expand = c(0,0))+
...
```
.source[Based on Riordan and Williamson, 1985]
...
- $\color{blue}{\Delta C}$: difference in production costs of organization (firm) vs. market
  - advantages of markets $>$ organization (firm)
  - declines as assets get more specific, but never fully reduced to zero (same as market)
  - since $\color{red}{\Delta C} > 0$, firm will never internalize for production cost reasons alone
]
]
...
ggplot(data = tibble(x = c(1,2)))+
  ...
  stat_function(fun = dg, color = "red")+
  geom_label(x = 2.75, y = dg(2.75), color = "red", label = expression(Delta*G))+
  stat_function(fun = dc, color = "blue")+
  geom_label(x = 2.75, y = dc(2.75), color = "blue", label = expression(Delta*C))+
  geom_vline(xintercept = 0, size = 0.5)+
  geom_hline(yintercept = 0, size = 0.5)+
  scale_x_continuous(breaks = c(0, 1.85),
                     labels = c(0, expression(k[1])),
...
- $\color{purple}{\Delta C + \Delta G}$: total cost difference between organization and market
]
]
...
ggplot(data = tibble(x = c(1,2)))+
  ...
  stat_function(fun = dc, color = "blue")+
  geom_label(x = 2.75, y = dc(2.75), color = "blue", label = expression(Delta*C))+
  stat_function(fun = total, color = "purple")+
  geom_label(x = 2.75, y = total(2.75), color = "purple", label = expression(Delta*G+Delta*C))+
  geom_vline(xintercept = 0, size = 0.5)+
  geom_hline(yintercept = 0, size = 0.5)+
  scale_x_continuous(breaks = c(0, 1.85, 2.25),
                     labels = c(0, expression(k[1]), expression(k[2])),
                     limits = c(0,3),
                     expand = c(0,0))+
  scale_y_continuous(breaks = c(0, 2.5),
                     labels = c(0, expression(Delta*G[0])),
...
.pull-left[
.smallest[
- At low levels of asset specificity $(k < k_2)$, market transactions are lower cost than organization
  - contract & buy most things
  - since $\color{red}{\Delta C} > 0$, firm will never internalize for production cost reasons alone

- At higher levels of asset specificity $(k > k_2)$, organization is lower cost than market
  - produce most things internally

- At modest levels of asset specificity $k_2$, no clear superior mode, may be a combination
...
base<-ggplot(data.frame(x=c(0,q_choke)), aes(x=x))+
  ...
  geom_segment(x=q_PC,y=0,xend=q_PC,yend=c,linetype="dotted", size=1)+
  ...
  labs(x = "Quantity (q)",
       y = "Price (p)",
       title = "(Downstream) Retailer's Market")+
  theme_classic(base_family = "Fira Sans Condensed", base_size=16)
base
```
]
...
.pull-right[
- $\color{red}{MC_R}$ for **Retailer** = price for **Manufacturer**’s output $(MC_R=p_M)$
...
ggplot(data.frame(x=c(0,q_choke)), aes(x=x))+
  ...
  stat_function(fun=MC_2, geom="line", size=2, color="darkred")+
  ...
  scale_x_continuous(breaks = c(0,3,4,q_PC),
                     labels = c(0,expression(q[2]^{R}), expression(q[1]^{R}),expression(q[PC])),
                     expand=c(0,0))+
  scale_y_continuous(breaks = c(0,c,c_2, Demand(4), Demand(3), a),
                     labels = c(0, expression(MC[1]^{R}==p[1]^{M}), expression(MC[2]^{R}==p[2]^{M}), expression(p[1]^{R}), expression(p[2]^R), expression(a)),
                     limits = c(0,a),
                     expand=c(0,0))+
  guides(color = F, fill = F)+
  labs(x = "Quantity (q)",
...
ggplot(data.frame(x=c(0,q_choke)), aes(x=x))+
  ...
  stat_function(fun=MC_2, geom="line", size=2, color="darkred")+
  stat_function(fun=MC_3, geom="line", size=2, color="pink")+
  ...
  geom_segment(x=3,y=0,xend=3,yend=Demand(3),linetype="dotted", size=0.5)+
      ...
      geom_segment(x=0,y=Demand(4.75),xend=4.75,yend=Demand(4.75),linetype="dotted", size=0.5)+
  geom_segment(x=4.75,y=0,xend=4.75,yend=Demand(4.75),linetype="dotted", size=0.5)+
  ...
  scale_x_continuous(breaks = c(0,3,4,4.75,q_PC),
                     labels = c(0,expression(q[2]^{R}), expression(q[1]^{R}), expression(q[3]^{R}), expression(q[PC])),
                     expand=c(0,0))+
  scale_y_continuous(breaks = c(0,c_3, c,c_2, Demand(4.75), Demand(4), Demand(3), a),
                     labels = c(0,expression(MC[3]^{R}==p[3]^{M}), expression(MC[1]^{R}==p[1]^{M}), expression(MC[2]^{R}==p[2]^{M}), expression(p[3]^{R}), expression(p[1]^{R}), expression(p[2]^{R}), expression(a)),
                     limits = c(0,a),
                     expand=c(0,0))+
  guides(color = F, fill = F)+
  labs(x = "Quantity (q)",
...
double<-ggplot(data.frame(x=c(0,q_choke)), aes(x=x))+
  ...
  # curves 
  stat_function(fun=Demand, geom="line", size=2, color="blue", alpha = 0.4)+
  stat_function(fun=MR, geom="line", size=2, color="purple")+
  stat_function(fun=MC, geom="line", size=2, color="darkred")+
  
  stat_function(fun=MR_M, geom="line", size=2, color="darkblue")+
  ...
  labs(x = "Quantity (q)",
       y = "Price (p)",
       title = "(Upstream) Manufacturer's Market")+
...
.pull-right[
- Since **Manufacturer**'s $\color{purple}{MR_M}$ curve is **Retailer's** Demand curve, consider  **Retailer**'s $\color{darkblue}{MR_R}$ curve
...
ggplot(data.frame(x=c(0,q_choke)), aes(x=x))+
  ...
  stat_function(fun=MR, geom="line", size=2, color="purple")+
  stat_function(fun=MC, geom="line", size=2, color="darkred")+
  geom_hline(yintercept=MR(2), size=2, color="red")+
  stat_function(fun=MR_M, geom="line", size=2, color="darkblue")+
...
double_m+
  ...
  geom_label(x = 1, y = 4, label = "Manuf. Profit", color = "darkgreen")+
  geom_label(x = 1, y = 7, label = "Retail Profit", color = "green")
...
- **Retailer** extracts some of this .hi-green[profit (darker)]
...
- Less total .hi-green[profit] (and it's split between Manufacturer & Retailer)
]
...
.pull-left[
.center[
![](../images/productionprocess2.png)

![](../images/productionprocess1.png)
]
]
