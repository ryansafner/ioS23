---
title: "4.4 — Contractual Restraints & Property Rights"
author: 'Ryan Safner'
output:
...
> “The particular circumstance we emphasize as likely to produce a serious threat of this type of reneging on contracts is the presence of appropriable specialized quasi rents. .hi[After a specific investment is made and such quasi rents are created, the possibility of opportunistic behavior is very real. Following Coase's framework, this problem can be solved in two possible ways: vertical integration or contracts],” (p.298)
...
- Recap: a contract removes parties from a competitive market and creates bilateral dependency between them
...
- Common law recognizes a contract to be **valid** if there is:
...
.smallest[Learn more in my [Economics of the Law](https://laws21.classes.ryansafner.com) course
]
]
]
...
- An aggrieved party can sue the other party for .hi-purple[breach of contract], and court will enforce judgment

- Courts typically require defendant to pay damages
  - may invalidate a contract
  - or require “specific performance” (very rare)
...
- A .hi[complete contract] (theoretically) specifies all actions or transfers that parties must take under every possible contingency

- In the real world of .hi-purple[uncertainty] (and transaction costs), complete contracts are *impossible* (and costly)
...
- Agreements are always .hi[incomplete contracts], actions for many (unforeseen) contingencies are left *unspecified*

- Even for *specified* actions and contingencies, outcomes are indeterminate due to .hi-purple[enforcement costs]
  - argument about facts, law, interpretation
...
.pull-right[
.smaller[
- .hi[Vertical integration] creates its own administrative & agency costs
  - And others we’ll see later today
  - Not always an efficient response to opportunism

- Alternative contractual solutions exist, often called .hi[“vertical restraints”], e.g.:
  - Exclusive dealing
...
.pull-left[
.center[
![](../images/franchise.jpg)
]
]

.pull-right[
.smaller[
- .hi[Franchise contract]: a .hi-red[franchisor] leases use of its key nontangible assets to .hi-blue[franchisee]

- .hi-red[Franchisor’s] non-tangible assets leased: business model, use of **brand** name, logos, trademarks, trade secrets, know-how, etc.

- .hi-blue[Franchisee] as entrepreneur owns/rents and manages physical assets (retail location, capital equipment, etc) & bears the risk

- .hi-blue[Franchisee] pays .hi-red[franchisor] **fees & royalties** to **license** .hi-red[franchisor’s] brand
]
]
...
.pull-left[
.center[
![](../images/franchising2.png)
]
]

.pull-right[
.center[
[![](../images/franchisebrands.png)](https://www.entrepreneur.com/franchises/topbrands)
]
]
...
.center[
[![:scale 70%](../images/mcdonaldsfranchise.png)](https://www.entrepreneur.com/franchises/mcdonalds/282570)
]
...
.pull-left[
.center[
![](../images/franchising.jpg)
]
]

.pull-right[

- Economies of scale with national (“corporate”) advertising, business strategy, name-recognition for all .hi-blue[franchisees] licensing the same brand

- Franchise contract contains many restrictions on what .hi-blue[franchisee] may do with .red[franchisor’s] brand, why?
]
...
- .hi-purple[“Horizontal” free riding:]
  - Many .hi-blue[franchisees] share the .hi-red[franchisor’s] brand name & reputation
  - Each .hi-blue[franchisee] will undersupply investment to maintain brand quality
]
...
- .hi-purple[“Vertical” free riding:]
  - .hi-red[Franchisor’s] income comes from fees and royalties from .hi-blue[franchisees], it’s *not* the residual claimaint
  - Each .hi-blue[franchisee] is a residual claimant, earns own profits off of their own sales (franchise fee is a cost to them)
  - .hi-red[Franchisor] will want to undersupply investments in advertising, strategy, quality provision (doesn’t fully internalize the value created)
]
...
.pull-right[
.quitesmall[
- .hi-red[Franchisor] has incentive to find efficient way to monitor itself and .hi-blue[franchisees] to maintain quality
  - As in Alchian and Demsetz (1972) and Jensen and Meckling (1976)!
  - .hi-red[Franchisor] can charge higher fees from a more valuable brand name

- Common stipulations in franchising agreements:
  - .hi-blue[Franchisees] may need to use specific equipment (e.g. certain shapes of buildings for fast food restaurants, hotels)
  - .hi-blue[Franchisees’] employees often need to go through common ("corporate") training programs to maintain quality and consistency
]
]
...
- Many contracts (including franchises) have .hi-purple[exclusive dealing] or .hi-purple[tie-in agreements] that require firm (e.g. .hi-blue[franchisee]) to purchase inputs *exclusively* from other firm (e.g. .hi-red[franchisor]) or a designated third-party

- Sound like market power/price discrimination?
...
.pull-left[
.center[
![](../images/breaking_bad_stay_out.gif)
...
- .hi-red[Franchisors] often include .hi-purple[territorial restraints] to create exclusive territories for .hi-blue[franchisees]

- Removes competition between .hi-blue[franchisees] in geographic area

- Avoids cannabalizing one another and diluting the .hi-red[franchisor’s] brand
]
...
.center[
![:scale 80%](../images/starbucksacross.jpg)
]
...
- A manufacturer in long-term contracts with retailers will often require .hi-purple[retail] or .hi-purple[resale price maintenance (RPM)]: 
...
.pull-left[
.ccenter[
![:scale 100%](../images/arbitration.png)
]
]
...
.pull-left[
![](../images/buyer_seller_PD.png)
]

.pull-right[

- Essentially firm and customer (which could be another firm) are playing an .hi-purple[infinitely repeated Prisoners’ Dilemma]
...
.pull-right[
.quitesmall[
- Consider the .hi-purple[“Grim” Trigger strategy]: cooperate, but once the other party plays Defect for the first time, Defect for all future turns
...
$$\begin{align*}
\frac{5}{1-\delta} & >  7+\frac{3\delta}{1-\delta}\\
\delta & > 0.50\
\end{align*}$$

- Sufficiently high discount rate/probability of future play constrains incentive to Defect
...
4. .hi[Property Rights View of the Firm]
...
- .hi[Agency theories] of the firm don’t distinguish between contracts *within* a firm vs. contracts *between* firms
  - The firm is just the legal .hi[“nexus” of many contracts] between various stakeholders of production
...
.pull-right[
.smaller[
- Any contract can specify who has .hi-purple[control rights under specific circumstances]
...
- *Purchased* firm (supplier) becomes a *division* of the *purchasing* firm
  - *Purchased* firm’s manager may become manager of the division
...
- If you know the .hi[Coase Theorem], it’s the same idea!
  - Resources tend to flow to those individuals/firms that value them the most, if there are clear and enforceable property rights, and transaction costs are low
